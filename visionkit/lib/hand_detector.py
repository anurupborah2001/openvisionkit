...
class HandDetector:
  ...
  def draw_landmarks(self,img_bgr, to_draw_landmark=True, to_draw_center_point=True, to_draw_bounding_box=True, to_put_handle_label=True):
    """
    ...
    # 👉 Get structured data
    all_hands = self.get_landmarks(img_bgr.copy())
  ...
  def get_landmarks(self, img):
    """
    Extracts landmarks, bounding boxes, and center points for each detected hand in the image.

    Args:
      img (numpy array): The image on which to detect hands.
  
    Returns:
      all_hands (list): A list of dictionaries containing landmarks, bounding boxes, center points, and hand types for each detected hand.
    """
  ...
  def fingers_up(self, hand_landmarks):
    ...
    for idx, handedness in enumerate(self.handedness_list):
      ...
      if hand_label == 'Right':
        if hand_landmarks[self.fingerTips[0]][1] > hand_landmarks[self.fingerTips[0] - 1][1]:  # For right hand
          fingers.append(1)
        else:
          fingers.append(0)
      else:
        ...
        if hand_landmarks[self.fingerTips[0]][1] < hand_landmarks[self.fingerTips[0] - 1][1]:  # For left hand
          fingers.append(1)
        else:
          fingers.append(0)
    ...
    for id in range(1, 5):
      if hand_landmarks[self.fingerTips[id]][2] < hand_landmarks[self.fingerTips[id] - 2][2]:
        fingers.append(1)
      else:
        fingers.append(0)
  ...
  def palm_width_px(self, img, hand_landmarks, drawLandmarks=False):
    ...
    index_mcp = (index_mcp_landmarks[1], index_mcp_landmarks[2])  # (x, y) for INDEX_MCP
    pinky_mcp = (pinky_mcp_landmarks[1], pinky_mcp_landmarks[2])  # (x, y) for PINKY_MCP
    if drawLandmarks:
  ...
  def _palm_scale(self, hand):
      ...
      return (
          self.euclidean_distance(hand[self.wrist], hand[self.finger_mcp[2]]) +   # wrist → middle MCP
          self.euclidean_distance(hand[self.finger_mcp[1]], hand[self.finger_mcp[4]])    # index MCP → pinky MCP
      ) / 2
  ...
  def estimate_distance_cm(self, palm_width_px,):
    """
    ##Formula
  ...
  def is_fingers_joined(self, p1, p2, image, landmarks, threshold=0.25, draw_intersection_point=True):
    """
  ...
  def is_fingers_joined_2(
    ...
    x1, y1, z1 = landmarks[p1][1:]
    x2, y2, z2 = landmarks[p2][1:]

    # distances
    norm_dist = self._normalize(landmarks, p1, p2)
    pixel_dist = self.euclidean_distance((x1, y1), (x2, y2))
    
    palm = self._palm_scale(landmarks)
    z_diff = abs(z1 - z2) / palm

    if debug:
        print(f"[DEBUG] norm={norm_dist:.4f}, pixel={pixel_dist:.2f}")

    # 🔥 REMOVE z condition (or normalize it properly)
    is_joined = (norm_dist < threshold and pixel_dist < min_pixel_dist)
  ...
  def is_thumbs_up(self):
    ...
    """
    fingers = self.fingers_up()
    return len(fingers) == 5 and fingers[0] == 1 and all(f == 0 for f in fingers[1:])
  ...
  def get_fingertip_positions(self, hand_landmarks):
    ...
    return {
      ...
      for name, tip in zip(names, self.fingerTips)
    }
...
def main():
  ...
  while True:
    ...
    fingers_up_list = handDetector.fingers_up()
    num_fingers = handDetector.count_fingers()
    ...
    cv2.imshow("Extended Hand Tracking (MediaPipe) - Tapping + Gestures", annotated_image)
